...
final class ObservatoryViewController: UIViewController {
    ...
    private func configureUI() {
        ...
        let title = UILabel(); title.text = "مركز الأرصاد V3.2"; title.font = .systemFont(ofSize: 24, weight: .bold); title.textColor = .white
    ...
    private func refresh() {
        ...
        let validKeys = ["journalValid","worldValid","observatoryValid","queueValid","weaponValid","ballisticsValid","damageValid","visibilityValid","tacticalAIValid"]
        ...
        frameBody.text = String(format: "FPS %.1f • AVG %.2fms\nP95 %.2f • P99 %.2f • MAX %.2f\n1%% LOW %.1f • 0.1%% LOW %.1f\nSlice AVG %.2f • MAX %.2fms • >20ms %@\n>20ms %@ • >33ms %@ • clamp %@", num(s,"estimatedFPS"),num(s,"averageFrameMs"),num(s,"p95FrameMs"),num(s,"p99FrameMs"),num(s,"maxFrameMs"),num(s,"onePercentLowFPS"),num(s,"pointOnePercentLowFPS"),num(s,"simulationSliceAverageMs"),num(s,"simulationSliceMaxMs"),str(s,"simulationSlicesOver20ms"),str(s,"framesOver20ms"),str(s,"framesOver33ms"),str(s,"catchUpClampedFrames"))
        inputBody.text = "Depth \(str(s,"queueDepth")) / Peak \(str(s,"queueHighWatermark"))\nCoalesced \(str(s,"queueCoalesced"))\nEvicted \(str(s,"queueEvicted"))\nRejected critical \(str(s,"queueRejectedCritical")) • invalid \(str(s,"queueRejectedInvalid"))\nDenials cooldown \(str(s,"denyFireCooldown")) • sprint \(str(s,"denyFireSprintRecovery"))"
        ...
        rendererBody.text = String(format: "Rendered %@ • misses %@\nCPU avg %.3fms • max %.3fms\nLock wait avg %.3fms • max %.3fms\nCore critical avg %.3fms • max %.3fms\nCallback gap max %.2fms • >250ms %@\nThermal %@ • %@ FPS", str(s,"renderedFrames"),str(s,"drawableMisses"),num(s,"renderCpuAverageMs"),num(s,"renderCpuMaxMs"),num(s,"coreLockWaitAverageMs"),num(s,"coreLockWaitMaxMs"),num(s,"coreCriticalAverageMs"),num(s,"coreCriticalMaxMs"),num(s,"callbackGapMaxMs"),str(s,"callbackGapsOver250ms"),thermal.name,str(s,"presentationFPS"))
    }
