...
final class ObservatoryViewController: UIViewController {
    ...
    private func refresh() {
        ...
        worldBody.text = String(format: "Obstacles %@ • Contacts %@\nVisibility F/R/M/D %@/%@/%@/%@\nAI active %@ • LOS %@ • hearing %@ • engaged %@\nOrder %@ • threat %.2f\nWorld %@ • AI %@", str(s,"worldObstacleCount"),str(s,"collisionContacts"),str(s,"visibilityFull"),str(s,"visibilityReduced"),str(s,"visibilityMinimal"),str(s,"visibilityDormant"),str(s,"aiActive"),str(s,"aiLOS"),str(s,"aiHearing"),str(s,"aiEngaged"),str(s,"aiSquadOrder"),num(s,"aiHighestThreat"),bool(s,"worldValid") ? "VALID":"FAIL",bool(s,"tacticalAIValid") ? "VALID":"FAIL")
