...
final class ObservatoryViewController: UIViewController {
    ...
    private func refresh() {
        ...
        worldBody.text = (worldBody.text ?? "") + "\nInjury recovery \(str(s,"aiRecovering")) • local loss concern \(str(s,"aiConcerned"))\nInjury reactions \(str(s,"aiInjuryReactions")) • witnessed pairs \(str(s,"aiWitnessedLosses"))\nWitness checks \(str(s,"aiLossChecks"))/\(str(s,"aiLossCheckCap")) • event truncations \(str(s,"aiLossBudgetDrops"))\nConsumed damage sequence \(str(s,"aiObservedDamageSequence"))"
