...
final class GameViewController: UIViewController {
    ...
    private func refreshStatus() { statusLabel.text = engine?.statusString() ?? "ENGINE OFFLINE"; guard let s = engine?.observatorySnapshot(), let stance = s["stance"] as? String else { return }; stanceButton.setImage(UIImage(systemName: stance == "PRONE" ? "figure.cooldown" : (stance == "CROUCH" ? "figure.strengthtraining.traditional" : "figure.stand")), for: .normal) }
