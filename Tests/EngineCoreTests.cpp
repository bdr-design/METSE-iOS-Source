...
ai.fixedStep(1.0/60.0,world,{0,0,0},{0,0,0},1.0);air=ai.report();assert(air.hearingAgents==1&&air.investigatingAgents==1&&air.squadOrder==AISquadOrder::Search);assert(ai.agents()[0].perceptionSource==AIPerceptionSource::Hearing);assert(dist3(ai.agents()[0].lastKnownPlayerPosition,{0,0,0})>0.1); // hearing-only estimate is intentionally inexact
for(int i=0;i<600;++i)ai.fixedStep(1.0/60.0,world,{0,0,0},{0,0,0},0.0);air=ai.report();assert(air.suspiciousAgents==0&&air.investigatingAgents==0&&air.engagedAgents==0&&air.squadOrder==AISquadOrder::Hold);assert(ai.agents()[0].perceptionSource==AIPerceptionSource::None);assert(ai.validate());
