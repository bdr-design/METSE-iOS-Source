...
struct Uniforms { float4 timing; float4 camera; float4 state; float4 character; };
...
fragment float4 metseFragment(VSOut in [[stage_in]], constant Uniforms& u [[buffer(0)]]) {
    ...
    float3 ray=normalize(forward + right*(uv.x*0.72) + up*((uv.y+pitch*0.72)*0.58)); float3 cam=float3(u.camera.x,u.character.x+u.character.y,u.camera.y);
    ...
    float moving=smoothstep(0.2,4.0,u.character.z); col += moving*0.0025*float3(0.04,0.10,0.07);
    return float4(col,1);
}
