...
struct GameplayDenialMetrics{std::uint64_t fireCooldown=0,fireReloading=0,fireObstructed=0,fireEmpty=0,projectileCapacity=0,reloadInvalid=0,autoReloadQueued=0;};
...
bool setMovementInput(double forward,double strafe)noexcept{return inputQueue_.pushMove(forward,strafe);}bool addLookInput(double yaw,double pitch)noexcept{return inputQueue_.pushLook(yaw,pitch);}bool setSprintHeld(bool held)noexcept{return inputQueue_.pushSprint(held);}bool setAimHeld(bool held)noexcept{return inputQueue_.pushAim(held);}bool cycleStance()noexcept{return inputQueue_.pushCycleStance();}
bool triggerFire()noexcept{const auto&w=weapon_.state();if(w.ammoInMagazine==0&&w.reserveAmmo>0&&!w.reloading){const bool queued=inputQueue_.pushReload();if(queued)++gameplayDenials_.autoReloadQueued;return queued;}return inputQueue_.pushFire();}
...
if(!precondition){
    if(kind==CommandKind::FireWeapon){const auto&w=weapon_.state();if(w.reloading)++gameplayDenials_.fireReloading;else if(w.obstructed)++gameplayDenials_.fireObstructed;else if(w.fireCooldown>0.0)++gameplayDenials_.fireCooldown;else if(w.ammoInMagazine==0)++gameplayDenials_.fireEmpty;else if(ballistics_.activeCount()>=BallisticsCore::kMaxProjectiles)++gameplayDenials_.projectileCapacity;else ++gameplayDenials_.fireCooldown;return false;}
