...
#include <cstdint>
namespace metse {
enum class WorldMaterial:std::uint8_t{Concrete=0,Steel,Wood};
...
struct WorldRayHit{bool hit=false;double t=1.0;Vec3 point{};WorldMaterial material=WorldMaterial::Concrete;std::size_t obstacleIndex=0;};
...
private:static bool overlaps(double v,double mn,double mx)noexcept;static double nearestBoundary(double previous,double desired,double mn,double mx)noexcept;static bool segmentAabb(Vec3 a,Vec3 b,const WorldObstacle&o,double&t)noexcept;
