...
void DamageCore::reset() noexcept {
    targets_ = {};
    targets_[0] = {1,{8.0,0.0,18.0},100.0,0.36,true};
    targets_[1] = {2,{-7.0,0.0,26.0},100.0,0.36,true};
    targetCount_ = 2;
    totalHits_ = 0;
    totalKills_ = 0;
    lastResult_ = {};
...
DamageResult DamageCore::applySegment(const Vec3& from,const Vec3& to,double energy,std::uint64_t correlationId) noexcept {
    DamageResult best{};
    double bestT=2.0;
    if (!std::isfinite(energy) || energy <= 0.0 || correlationId==0) return best;
    for (std::size_t i=0;i<targetCount_;++i) {
        auto& target=targets_[i];
        if (!target.alive) continue;
        double t=0.0;
        if (segmentPointDistanceXZ(from,to,target.position,t)>target.radius || t>=bestT) continue;
        const double hitY=from.y+(to.y-from.y)*t-target.position.y;
        if (hitY < 0.15 || hitY > 1.88) continue;
        HitRegion region=HitRegion::Limb;
        double multiplier=0.40;
        double armorThreshold=0.0;
        if (hitY >= 1.55) { region=HitRegion::Head; multiplier=1.35; armorThreshold=320.0; }
        else if (hitY >= 1.02) { region=HitRegion::Thorax; multiplier=0.78; armorThreshold=610.0; }
        else if (hitY >= 0.66) { region=HitRegion::Abdomen; multiplier=0.62; }
        ...
        target.alive=!killed;
        ++totalHits_;
        if (killed) ++totalKills_;
        best={true,killed,target.id,region,damage,target.health,correlationId};
        lastResult_ = best;
        ++resultSequence_;
        bestT=t;
    }
