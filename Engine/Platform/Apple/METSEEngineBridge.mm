...
- (NSDictionary<NSString *, id> *)observatorySnapshot {
    ...
    return @{
        ...
        @"aiShotsFiredDiagnostics":@(d.aiShotsFired), @"aiTargetImpactsDiagnostics":@(d.aiTargetImpacts), @"friendlyFireDenialsDiagnostics":@(d.friendlyFireDenials), @"combatantRecords":@(d.combatants.count()),
...
- (NSString *)observatoryReportText {
    ...
    [report appendFormat:@"Combatant Authority total %@ | player HP %.1f state %@ bleed %.2f/s | AI shots %@ player impacts %@ friendly-fire denials %@\n",
        s[@"combatantCount"],[s[@"playerHealth"] doubleValue],s[@"playerCombatState"],[s[@"playerBleedingPerSecond"] doubleValue],
